...
from typing import Awaitable, TypeAlias, Union, TypedDict
...
Autofill: TypeAlias = Union[str,
                            int,
                            Callable[[], str],
                            Callable[[], Awaitable[int | str]],
                            Queue]
...
class Client(_Client):
    ...
    def __init__(self, *args,
                 ...
                 proxy: Proxy | ProxyDict = None,
                 **kwargs):
        name = name or f"{kwargs.get('api_id')}_{phone_number}"
        proxy = proxy.to_dict() if isinstance(proxy, Proxy) else proxy
        super().__init__(*args,
                         name,
    ...
    async def set_unfilled_attribute(self, attr_name: str) -> str | int | None:
